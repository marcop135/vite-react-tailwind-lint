...
Do not re-scope `release:check` to the full tree. A dev-only advisory with no published fix would then block every PR, every scheduled npm update, and every release at once, and no single dependency PR could clear it. Clear dev advisories with `npm run audit:fix`. Reach for `overrides` in `package.json` only when no upstream fix exists, and use a caret range, never an exact pin: an exact pin at a version that later turns vulnerable cannot be lifted by `npm update`, which is exactly how the `brace-expansion: 5.0.8` override wedged this repo.
...
- `scheduled-npm-update.yml` enables auto-merge on its own PR (same PAT, same reason), so in-range bumps land unattended and the next scheduled release ships them. Major bumps are the only dependency work left for a human.
...
- Add inline `<script>` to `index.html`; the Netlify CSP is `script-src 'self'`.
